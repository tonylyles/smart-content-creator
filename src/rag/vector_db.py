...
class VectorDB:
            ...
            try:
                ...
                if api_key:
                    self._embedding = OpenAIEmbeddings(
                        model=self.config.get("rag", {}).get(
                            "embedding_model", "text-embedding-ada-002"
                        ),
                        openai_api_key=api_key,
                        openai_api_base=gen_config.get(
    ...
    def add_documents(self, documents, collection=None):
        ...
        Args:
            documents: 文档列表，格式为 [{"title", "content", "category", "tags", "source"}].
            collection: 集合名称（可选）.
        ...
        for doc in documents:
            ...
            if self._has_qdrant and self._get_embedding():
                collection_name = (
                    collection
                    or self.config.get("rag", {}).get("collection", "smart_content")
                )
